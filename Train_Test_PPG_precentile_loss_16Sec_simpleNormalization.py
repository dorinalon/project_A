...
r_path = only_PPG_path + "/saved_nn_Ron"
...
def calc_loss_precentage(expec_BP, pred_BP, time_length, iteration_num, patient_num):
    ...
    for i in range(int(time_length/interval_30_sec) ):
        ...
        pred_95.append(np.percentile(pred_BP[i * interval_30_sec:(i + 1) * interval_30_sec], 97))
    ...
    plt.savefig(only_PPG_path + '/figures' + '/Prediction and Expected BP systolic and diastolic ' + str(iteration_num))
...
def Train_and_Test_4signals(path_bp, path_ppg, path_ecg, path_ri, patient_num, train_start_time):
    ...
    train_bp_scaled = train_bp#scaler_BP.fit_transform(train_bp)
    ...
    train_ppg_scaled = train_ppg#scaler_PPG.fit_transform(train_ppg)

    test_bp_scaled = test_bp#scaler_BP.transform(test_bp)
    # test_ecg_scaled = scaler_ECG.fit_transform(test_ecg)
    # test_ri_scaled = scaler_RI.fit_transform(test_ri)
    test_ppg_scaled = test_ppg#scaler_PPG.fit_transform(test_ppg)
    ...
    if enableBatch:
        ...
        out = out.float().cuda()
        #inp_ppg = torch.Tensor(train_ppg_scaled.reshape(2**12, -1, 1)).cuda()
        # inp_ecg = torch.Tensor(train_ecg_scaled.reshape((train_ecg_scaled.shape[0], -1, 1))).cuda()
        #out = torch.Tensor(train_bp_scaled.reshape(2**12, -1, 1)).cuda()
    else:
    ...
    if enableTrain:
        for t in range(10000):
            ...
            optimizer.zero_grad()
            pred = r((inp_ppg-2200)*1e-3)
            predictions.append(pred.data.cpu().numpy())
            loss = loss_func(pred, (out-2400)*1e-3)
            if not(changedFlag):
                if loss.item() < 3000:
            ...
            if t%50==0:
                ...
                if True:#enableBatch:
                    plt.plot(( -40 + 0.0625*(2400+1e3*(pred[:1000, 0, 0])) ).data.cpu().numpy(), label='prediction_BP')
                    plt.plot(( -40 + 0.0625*(out[:1000, 0, 0]) ).data.cpu().numpy(), label='train_BP')
                ...
                plt.legend()
                plt.savefig(only_PPG_path + '/figures' + '/iteration_' + str(t) +" from minute "+ str(train_start_time))
                ...
                plt.legend()
                plt.savefig(only_PPG_path + '/figures' + '/Prediction and Expected BP ' + str(t)+" from minute "+str(train_start_time))
                '''
        # Save the net
        ...
        plt.ylabel('loss')
        plt.savefig(only_PPG_path + '/figures' + '/Mean_loss'+" from minute "+ str(train_start_time))
    else:
        #r.load_state_dict(torch.load('./nn_16Sec.pt'))
        #r.load_state_dict(torch.load('./nn_16Sec_2728529_6534.pt'))
        r.load_state_dict(torch.load('./nn_16Sec_2677398-3042_0_8_min.pt'))
        #r.load_state_dict(torch.load(r_path))

    # TEST- on the same patient as the train

    pred_t = -40 + 0.0625*(1e3*(r((x_test_PPG-2200)*1e-3))+2400)
    ...
    pred_t = pred_t[:, :,0].data.cpu().numpy()#scaler_BP.inverse_transform(pred_t[:, :,0].data.cpu().numpy())
    test_bp_scaled = test_bp_scaled#scaler_BP.inverse_transform(test_bp_scaled)[:,:]
    ...
    plt.plot(-40 + 0.0625*test_bp[:1000,0], label ='expected_BP')
    plt.xlabel('samples')
    plt.ylabel('mmHg')
    plt.ylabel('mmHg')
    plt.title(" BP Prediction based on PPG - original values")
    plt.legend()
    plt.savefig(only_PPG_path + '/figures' + '/FINAL ZOOM: Prediction and Expected BP from minute '+str(train_start_time))

    plt.clf()
    plt.plot(pred_t[:,0], label='prediction_BP')
    plt.plot(-40 + 0.0625*test_bp[:,0], label='expected_BP')
    plt.xlabel('samples')
    plt.ylabel('mmHg')
    plt.ylabel('mmHg')
    plt.title(" BP Prediction based on PPG - original values")
    plt.legend()
    plt.savefig(only_PPG_path + '/figures' + '/FINAL: Prediction and Expected BP from minute ' + str(train_start_time))
    plt.clf()

    calc_loss_precentage(-40 + 0.0625*test_bp[:, 0], pred_t[:, 0], test_size_125Hz, 'orig value from minute '+str(train_start_time), patient_num)
...
'''
Train_and_Test_4signals('./2728529-6534-MDC_PRESS_BLD_ART_ABP-125.csv',
                        './2728529-6534-MDC_PULS_OXIM_PLETH-125.csv',
                        './2728529-6534-MDC_PULS_OXIM_PLETH-125.csv',
                        './2728529-6534-MDC_PULS_OXIM_PLETH-125.csv',
                       './2728529-6534/',12)

Train_and_Test_4signals('./2728529-6527-MDC_PRESS_BLD_ART_ABP-125.csv',
                        './2728529-6527-MDC_PULS_OXIM_PLETH-125.csv',
                        './2728529-6527-MDC_PULS_OXIM_PLETH-125.csv',
                        './2728529-6527-MDC_PULS_OXIM_PLETH-125.csv',
                       './2728529-6527/',0)


Train_and_Test_4signals('./2677398-3042-MDC_PRESS_BLD_ART_ABP-125.csv',
                        './2677398-3042-MDC_PULS_OXIM_PLETH-125.csv',
                        './2677398-3042-MDC_PULS_OXIM_PLETH-125.csv',
                        './2677398-3042-MDC_PULS_OXIM_PLETH-125.csv',
                       './2677398-3042/',0)
'''
Train_and_Test_4signals('./2677398-3043-MDC_PRESS_BLD_ART_ABP-125.csv',
                        './2677398-3043-MDC_PULS_OXIM_PLETH-125.csv',
                        './2677398-3043-MDC_PULS_OXIM_PLETH-125.csv',
                        './2677398-3043-MDC_PULS_OXIM_PLETH-125.csv',
                       './2677398-3043/',26)
